## Installation et maintenance d'un NAS Samba sur Raspberry Pi 5

1. Copier & sauvegarder le script :

```bash
sudo nano install_nas.sh
```

(colle le contenu, `CTRL+X`, `O`, `ENTRÉE`)
...
for user_dir in $MOUNT_POINT/*/; do
    ...
    if [[ "$dirname" != "public" && "$dirname" != "prive" ]]; then
        echo "   - $MOUNT_POINT/$dirname (RW utilisateur $dirname uniquement)"
    fi
done
echo ""
echo "🌐 Accès réseau :"
